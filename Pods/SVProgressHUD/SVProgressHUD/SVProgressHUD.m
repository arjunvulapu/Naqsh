...
- (instancetype)initWithFrame:(CGRect)frame {
    if((self = [super initWithFrame:frame])) {
        ...
        _backgroundLayerColor = [UIColor colorWithWhite:0 alpha:0];
...
+ (void)showAnimatedImages {
    [[self sharedView] showAnimatedImagesNonStatic];
}

- (void)showAnimatedImagesNonStatic {
    __weak SVProgressHUD *weakSelf = self;
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        __strong SVProgressHUD *strongSelf = weakSelf;
        if(strongSelf){
...
//                [images addObject:[UIImage imageNamed:@"flag.png"]];
//            }
//            
//            strongSelf.imageView.animationImages = images;
//            strongSelf.imageView.animationDuration = 1;
//            strongSelf.imageView.hidden = NO;
//            [strongSelf.imageView startAnimating];
            
            strongSelf.imageView.image = [UIImage imageNamed:@"flag.png"];
            strongSelf.imageView.hidden = NO;
...
- (void)showImage:(UIImage*)image status:(NSString*)status duration:(NSTimeInterval)duration {
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if(strongSelf){
            ...
            strongSelf.imageView.animationImages = images;
            strongSelf.imageView.animationDuration = 0.5;
            strongSelf.imageView.hidden = NO;
            [strongSelf.imageView startAnimating];
...
- (UIView*)hudView {
    ...
    _hudView.backgroundColor = [UIColor clearColor];//self.backgroundColorForStyle;
