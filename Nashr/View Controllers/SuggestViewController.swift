...
class SuggestViewController: BaseViewController {
    ...
    override func viewDidLoad() {
        ...
        self.navigationItem.title = Localization.get("title_suggest")
    ...
    @IBAction func sendMessage(sender: AnyObject) {
        if self.editName.text?.isEmpty == true {
            ...
            makeCall(Page.suggestions, params: ["name":self.editName.text!, "email":self.editEmail.text!, "message":self.textMessage.text!], completionHandler: { (response) in
