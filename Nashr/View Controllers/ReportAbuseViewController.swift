...
class ReportAbuseViewController: BaseViewController {
    ...
    override func viewDidLoad() {
        ...
        self.navigationItem.title = Localization.get("title_report_abuse")
    ...
    @IBAction func sendMessage(sender: AnyObject) {
        if self.editName.text?.isEmpty == true {
            ...
            makeCall(Page.reportAbuse, params: ["news_id":self.newsId!, "name":self.editName.text!, "email":self.editEmail.text!, "message":self.textMessage.text!], completionHandler: { (response) in
